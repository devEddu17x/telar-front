...
describe('CreateOrderDialog', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (useRouter as jest.Mock).mockReturnValue({ refresh: mockRefresh, push: mockPush });
  });
  ...
  it('debería mostrar errores de validación si se envía vacío', async () => {
    ...
    const submitBtn = within(dialog).getByRole('button', { name: 'Crear orden', hidden: true });
    
    await user.click(submitBtn);

    await waitFor(() => {
      expect(screen.getByText('La fecha de entrega es requerida')).toBeInTheDocument();
